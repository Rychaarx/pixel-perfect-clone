...
// Tenta girar a tela nativamente (Android + Safari 16.4+)
...
const TitleDetails = () => {
  ...
  // Desbloqueia orientação ao fechar player
  useEffect(() => {
    if (!watching) {
      unlockOrientation();
  ...
  // Detecta rotação manual do iPhone
  useEffect(() => {
    if (!watching) return;
    const handle = () => {
      if (nativeRotation) {
  ...
  if (watching) {
    ...
    // Link externo não embeddável → abre em nova aba
    ...
    // Se a API girou nativamente, não precisa de CSS rotate
