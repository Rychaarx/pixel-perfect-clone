...
import { ArrowLeft, Star, Clock, Play, Heart, Share2, Maximize, Minimize } from "lucide-react";
...
// Tenta girar a tela via Screen Orientation API (Android + Safari 16.4+)
// Fallback: retorna false para usar o CSS rotate
...
const MovieDetail = () => {
  ...
  const [nativeRotation, setNativeRotation] = useState(false); // true = API girou, false = CSS rotate
  ...
  // Quando o player fecha, desbloqueia orientação
  useEffect(() => {
    if (!playing) {
      unlockOrientation();
  ...
  // Detecta se o usuário girou o iPhone manualmente
  useEffect(() => {
    if (!playing) return;
    const handleOrientationChange = () => {
      const landscape = window.matchMedia("(orientation: landscape)").matches;
      if (nativeRotation) setIsLandscape(landscape);
    };
  ...
  const handleOpen = () => setPlaying(true);
  ...
  // Se a API girou nativamente, o vídeo já está landscape — não precisa de CSS rotate
  const videoContainerStyle: React.CSSProperties = (!nativeRotation && isLandscape)
    ? {
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      {playing && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "#000", display: "flex", flexDirection: "column" }}>
          ...
          {/* Barra superior — esconde no landscape nativo pra maximizar tela */}
          {!nativeRotation && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "#111", flexShrink: 0, minHeight: 52 }}>
              ...
              <button onClick={handleToggleLandscape}
          ...
          {/* Botão flutuante quando está em landscape nativo */}
          {nativeRotation && (
            <div style={{ position: "absolute", top: 12, right: 12, zIndex: 10000, display: "flex", gap: 8 }}>
              <button onClick={handleClose}
                style={{ color: "#fff", background: "rgba(0,0,0,0.6)", border: "none", borderRadius: 20, padding: "8px 14px", fontSize: 13, cursor: "pointer" }}>
                ✕
              </button>
              <button onClick={handleToggleLandscape}
                style={{ color: "#fff", background: "rgba(0,0,0,0.6)", border: "none", borderRadius: 20, padding: "8px 14px", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                <Minimize size={14} /> Vertical
              </button>
            </div>
          )}
          ...
          {/* Barra de progresso — esconde no landscape nativo */}
          {!nativeRotation && (
